...
export async function probeRecordingPersistence(
  ...
  if (
    ...
    return {
      ...
      outcome: "skipped",
      message:
        "NOT MEASURED: the local SQLite store is behind the current schema, and writing to it " +
        "would apply pending migrations to a legacy file. Migrate deliberately " +
        "('recordings migrate') or re-run with --probe-store-write to accept the migration as " +
        "part of the probe.",
    };
  }
